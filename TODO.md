...
- [ ] **F2 — Continuous (non-turn-based) streaming.** Chunk/translate/stream TTS while the speaker keeps talking, without breaking the sentence-boundary buffering requirement. Core open engineering problem — see design doc's open questions. Architecture note from the 2026-09 review: the current single-thread design (translate blocking the capture loop, piper subprocess inside the Player thread) won't survive streaming — move to explicit bounded queues between stages with backpressure decisions (drop/merge/delay); whether the stages become OS processes, threads, or local services is an F2 design decision, decided then, not now.
...
- [x] **Soak/latency instrumentation built (P2/P1, 2026-09-14)**: `live_loop.py` now logs per-utterance ASR/MT/end-to-end timings, per-sentence playback wait + audio duration (`SPK [...]`), working-set memory at start/every 5min/end, and a session summary on exit. **Still to do: capture a real baseline** from a 10–15 min session into `validation/` so F2 has measured numbers to design against.
