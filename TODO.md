...
- [ ] **F2 — Continuous (non-turn-based) streaming.** Design approved (design doc section 13). **Build steps 1–4 implemented (2026-09-14), all tests green** — pipeline refactored to bounded queues (segmenter → ASR+stitcher → MT → TTS synth → bounded playback queue → Player), playback backlog tracked as a metric (max reported in stats), Piper `--length-scale` (default 0.9), rule-based disfluency stripping before MT (`--no-strip` to disable; unit-tested; "like"/"so" deliberately preserved), and catch-up mode behind `--catchup` (default **off**) with `--backlog-threshold 15` dropping oldest un-played sentences down to threshold/2 (hysteresis). **Remaining: step 5 — re-run the two baseline scenarios** (improvised + read-aloud) with and without `--catchup`, compare against `validation/latency/2026-09-14-continuous-speech.md`. Needs Jerry at the mic.
