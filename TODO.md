...
- [ ] **F2 — Continuous streaming.** Incremental ASR + catch-up Tiers 0/1 + transcript logging built. 2026-09-15 three-run session (validation/latency/2026-09-15-runs.md) confirmed incremental ASR works (E2E down to ~1.7–2.2s) and exposed two bugs, both fixed same day: revision-duplicate sentences reaching MT twice, and giant unpunctuated fragments becoming ~60s TTS blobs (the real cause of the --catchup drops and the 39.7s default-mode backlog — not the flag itself). **Remaining: one clean replay** of the dense video (default + --catchup) to confirm no dupes, no blobs, fewer drops; then audible drop cue; then F2 closes. Developer mechanics doc: docs/HOW-IT-WORKS.md (update it in the same commit as pipeline changes).
...
- [ ] **NCAIR N-ATLaS model family — African-language candidates (design 6a).**
  - **`NigerianAccentedEnglish` — downloaded + converted (2026-09-17), ready for live testing.** Whisper-small fine-tune on 120h Nigerian-accented English. Lives at `validation/models/faster-whisper-small-en-ng/` (CTranslate2 int8, 248MB); use as `live_loop.py --asr-model small-en-ng` (existing flag maps to the dir, no code change). Smoke test on synthesized speech beat stock `small` on the M4 case: "naira" transcribed correctly (stock: "narrow" + repeated phrase), "Abuja" near-miss ("abuge"). Note: hallucinated a sentence on pure silence — same filler-hallucination class as the Whisper filter TODO. License caution for F8: custom Awarri license, 1,000-active-end-user cap, "Powered by Awarri" derivative naming, gated repo — fine for validation, needs a licensing decision before bundling in the installer.
  - `Igbo-ASR` covers Igbo as a source language — not yet evaluated.
  - `N-ATLaS` (Llama-3 8B, Hausa/Igbo/Yoruba + accented English): **effectively no-go as MT** (LLM-MT reintroduces §8 hallucination risk; 8B per-sentence latency blows the budget; self-published eval; Yoruba scored 2.69/5). Remainder of interest: parked Tier-2 summarizer candidate only, gated on Tiers 0/1 proving insufficient + fragment go/no-go + Llama-3/Awarri license check. Community quants exist (GGUF Q4/Q8, FP8). https://huggingface.co/NCAIR1
  - `Sunbird/asr-whisper-51-african-languages` (large-v3, ~2B): broad African-language ASR but ~8x the compute of small-class models — likely breaks the CPU real-time budget (baseline small = RTF 0.15; large ≈ 1.0+ on CPU). Revisit only if F3 multi-language events demand languages the NCAIR models don't cover, and after the whisper.cpp/Vulkan GPU path exists.
