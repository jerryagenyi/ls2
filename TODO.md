...
- [ ] **NCAIR N-ATLaS model family — candidates for the African-language pass (design 6a).** `NigerianAccentedEnglish` (Whisper-small fine-tune) directly targets Jerry's accent concern and should convert to faster-whisper/CTranslate2 and drop into the ASR slot unchanged; `Igbo-ASR` covers Igbo as a source language; `N-ATLaS` (Llama-3 8B, Hausa/Igbo/Yoruba + accented English) is a *candidate* for low-resource MT and/or the parked Tier-2 summarizer — LLM-MT reintroduces the section-8 hallucination risk, so it must pass the fragment go/no-go first, and check the Llama-3 license against commercial packaging. Self-published eval = claim, not evidence. Evaluate in a dedicated session. https://huggingface.co/NCAIR1
