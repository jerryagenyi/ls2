...
- [ ] **Soak/latency instrumentation** (test-plan §1.3): 10-minute no-human soak watching memory; per-utterance latency logging (utterance end → EN → FR → TTS queued → playback start) so F2 has a measured baseline before the ≤5s target means anything.
