...
- [ ] **F2 — Continuous (non-turn-based) streaming.** Chunk/translate/stream TTS while the speaker keeps talking, without breaking the sentence-boundary buffering requirement. **Design drafted (2026-09-14, design doc section 13) — awaiting Jerry's review before build.** Core reframe from the baseline: it's a compression problem, not a speed problem (catch-up mode with bounded playback queue, TTS speedup, disfluency stripping). Build order in section 13.6.
