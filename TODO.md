...
- [ ] **F2 — Continuous (non-turn-based) streaming.** Design approved (design doc section 13); steps 1–4 built; playback wiring bug found and fixed on first live run. **YouTube replays done (2026-09-14, `validation/latency/2026-09-14-youtube-run.md`): catch-up mode verified** — backlog bounded at 12.5s vs 30.9s without it, 9 drops, waits capped. **Next F2 increment: incremental in-utterance ASR** — dense speech has no ≥0.8s pauses, so the first utterance only closes at the 30s force-flush (French started ~30s into the video); transcribe the growing buffer every few seconds and emit sentences as they complete. Also add the time-from-speech-start metric, and the audible drop cue from design 13.4 before catch-up is used at any real event.
