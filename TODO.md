...
- [ ] **F2 — Continuous (non-turn-based) streaming.** Design approved (design doc section 13). Build steps 1–4 implemented; first live run (YouTube audio, `validation/latency/2026-09-14-youtube-run.md`) caught a real wiring bug — TTS output queued to a queue the Player never drained, so no French audio — **fixed** (Player now owns the bounded playback queue); all 31 tests green. **Remaining: step 5 — re-run the scenarios** (YouTube video + read-aloud) with and without `--catchup`, compare against the baselines. Needs Jerry at the mic.
